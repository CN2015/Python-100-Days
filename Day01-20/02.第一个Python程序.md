...
**补充**
在PyCharm中，注释代码是一种常用的操作，它可以帮助开发者临时禁用某些代码或者添加说明。PyCharm提供了快捷键来实现单行或多行的快速注释。

1.单行注释
要对单行代码进行注释，你可以简单地将光标放在该行上，然后使用快捷键Ctrl + /。这将在该行代码前添加一个#符号，从而将其注释掉。如果你已经选中了一行或多行代码，同样的快捷键也会对所有选中的行进行注释。
...
```
2.多行注释
如果你想要注释多行代码，可以通过鼠标拖拽选中这些行，然后按下Ctrl + /快捷键。这样，每行代码前都会添加#符号，实现多行注释。
...
三引号注释
PyCharm还支持使用三引号('''或""")进行块注释。虽然这通常用于多行字符串，但也可以用作多行注释。你可以选中要注释的代码块，然后按Shift键并连续三次按引号键('或")，这样就可以将选中的代码块放在三引号之间。
...
取消注释
如果你需要取消注释，无论是单行还是多行，都可以使用相同的Ctrl + /快捷键。只需选中已注释的代码，然后按下快捷键，注释符号就会被移除。
...
通过这些快捷键，你可以提高编码效率，快速地对代码进行注释或取消注释。记住，使用快捷键时不需要选中整行代码，只需将光标放在相应的行上即可。
